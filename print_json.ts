...
async function run() {
  ...
  const file = {
    buffer: pdfBuffer,
    originalname: 'E1 ESOL Reading Candidate Paper PPA.pdf',
...
4. If it's a Tutor Copy with printed answers, set the correctIndex.`;
  const jsonStr = await service.extractPdf(prompt, file);
  fs.writeFileSync('output.json', jsonStr.responseText);
}
run();
