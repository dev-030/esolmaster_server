...
async function testRun() {
  ...
  const filePath = path.join(__dirname, '../khalaf79-attachments/E1 ESOL Reading Candidate Paper PPA.pdf');
  const buffer = fs.readFileSync(filePath);
  const mockFile = { buffer, originalname: 'PPA.pdf', mimetype: 'application/pdf', size: buffer.length };
